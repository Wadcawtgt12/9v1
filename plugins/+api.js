...
switch (command) {
case 'menu': case 'help': case 'tiachannn':
...
case 'infobot':
if (!iselaina) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			reply('Update bot selanjutnya silahkan tunggu')
break
case 'owner':{
if (!iselaina) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
case 'tiktokmusic': case 'tiktokaudio':  
			...
			.then(result => {
             ...
             break
case 'jadwaltv':
if (args.length == 0) return reply(`Nama Channel Nya\nContoh: ${prefix + command} SCTV`)
channel = args[0]
...
case 'memegenerator': case 'memegen':{
						...
						if (args.length < 1) return reply(`Kirim perintah *${prefix + command}* bot`)
									if (q.includes('|')) return reply(`Kirim perintah *${prefix + command}* bot`)
									try {
										if (!isQuotedImage) return reply(`Reply Gambar!`)
										reply(lang.wait())
										var teks2 = args.join(' ')
...
case 'sewabot':
if (!iselaina) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			reply('http://wa.me/62895411143170?text=Sewa+Bot') 		
break
case 'kick':
			if (!isGroup) return reply(lang.group())
			if (!isGroupAdmins) return reply(lang.admin(groupName))
...
case 'ppcp': 
       ...
       cewe = await getBuffer(anu.result.cewe_image)
		    pll = `Ini Untuk  **`
   sendButImage(from, pll, `Bot Ofc  2022`, cewe, [                      
          {
